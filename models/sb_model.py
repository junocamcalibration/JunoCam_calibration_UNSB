...
class SBModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--nce_idt_A', action='store_true', default=False, help='use NCE loss for identity mapping: NCE(G(X), X))')
    ...
    def __init__(self, opt):
        ...
        # Add identity A loss
        if opt.nce_idt_A and self.isTrain:
            self.loss_names += ['NCE_X']
            self.visual_names += ['idt_A']
    ...
    def set_input(self, input,input2=None):
        ...
        if self.opt.netG == "resnet_9blocks_cond_fourierfeat":
            self.fourierfeat = input['fourierfeat'].to(self.device)


    def run_G(self, X, time, z, layers=[], encode_only=False):
        if self.opt.netG == "resnet_9blocks_cond_fourierfeat":
            return self.netG(X, time, z, self.fourierfeat, layers, encode_only)
        else:
            return self.netG(X, time, z, layers=layers, encode_only=encode_only)
    ...
    def forward(self):
        ...
        with torch.no_grad():
            ...
            for t in range(self.time_idx.int().item()+1):
                ...
                if self.opt.nce_idt:
                    ...
                    time     = times[time_idx]
                    z        = torch.randn(size=[self.real_A.shape[0],4*self.opt.ngf]).to(self.real_A.device)
                    Xt_1B = self.run_G(X=XtB, time=time_idx, z=z)
        ...
        self.fake = self.run_G(X=self.realt, time=self.time_idx, z=z_in)

        self.fake_B2 = self.run_G(X=self.real_A_noisy2, time=self.time_idx, z=z_in2)
        ...
        if self.opt.nce_idt_A:
            self.idt_A = self.fake_B.clone()
    ...
    def compute_G_loss(self):
        ...
        # Add identity A loss
        if self.opt.nce_idt_A and self.opt.lambda_NCE > 0.0:
            self.loss_NCE_X = self.calculate_NCE_loss(self.real_A, self.idt_A)
            loss_NCE_both += self.loss_NCE_X
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        feat_q = self.run_G(X=tgt, time=self.time_idx*0, z=z, layers=self.nce_layers, encode_only=True)

        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]
        
        feat_k = self.run_G(X=src, time=self.time_idx*0, z=z, layers=self.nce_layers, encode_only=True)
