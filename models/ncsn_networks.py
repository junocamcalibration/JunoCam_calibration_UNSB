...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            self.model_res += [ResnetBlock_cond(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias,temb_dim=4*ngf,z_dim=4*ngf, fix_bug=opt.fix_time_bug)]
...
class DPT_Generator(nn.Module):

    def __init__(self, input_nc, output_nc, ngf=128, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect', opt=None):

        super(DPT_Generator, self).__init__()
        ...
        if type(norm_layer) == functools.partial:
            ...
            use_bias = norm_layer == nn.InstanceNorm2d
        mult = 2 # channel upsample factor 

        # Define main DPT model
        # Params for DPT are mostly the default ones
        # backbone= "vitb_rn50_384", "vitl16_384", "vitb16_384"
        self.model = DPT(backbone=opt.dpt_backbone)

        # Define the conditional resnet conv blocks in order to fuse the time conditional and z
        self.model_res = nn.ModuleList()
        for i in range(n_blocks):
            self.model_res += [ResnetBlock_cond(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, 
                                    use_dropout=use_dropout, use_bias=use_bias,temb_dim=2*ngf,z_dim=2*ngf, fix_bug=opt.fix_time_bug)]

        # Define upsampling head
        model_upsample = []
        model_upsample += [
            Upsample(ngf * mult),
            nn.Conv2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=1, padding=1, bias=use_bias),
        ...
        # Define the z embedding net
        mapping_layers = [PixelNorm(),
                      nn.Linear(self.ngf*2, self.ngf*2),
                      nn.LeakyReLU(0.2)]
        for _ in range(opt.n_mlp):
            mapping_layers.append(nn.Linear(self.ngf*2, self.ngf*2))
            mapping_layers.append(nn.LeakyReLU(0.2))
        self.z_transform = nn.Sequential(*mapping_layers)

        # Define the time embedding net
        modules_emb = []
        modules_emb += [nn.Linear(self.ngf,self.ngf*2)]
        nn.init.zeros_(modules_emb[-1].bias)
        modules_emb += [nn.LeakyReLU(0.2)]
        modules_emb += [nn.Linear(self.ngf*2,self.ngf*2)]
        nn.init.zeros_(modules_emb[-1].bias)
        modules_emb += [nn.LeakyReLU(0.2)]
        self.time_embed = nn.Sequential(*modules_emb)
    ...
    def forward(self, x, time_cond, z, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            # Use the following layers to collect features:
            # DPT model: scratch.refinenet1, scratch.refinenet2
            # Model res: use resnet_cond blocks defined in the layers argument 
            feat = x
            feats = []
            feat, out_2 = self.model(feat)
            feats.append(feat)
            feats.append(out_2)
...
class ResnetGenerator_ncsn_FourierFeat(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetGenerator_ncsn_FourierFeat, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        # Model to downsize the fourier features
        self.ffmodel = nn.Sequential(
                        nn.Conv2d(
                            256,
                            256,
                            kernel_size=1,
        ...
        # Model to fuse fourier feat with output of resnet
    ...
    def forward(self, x, time_cond, z, fourierfeat=None, layers=[], encode_only=False):
        z_embed = self.z_transform(z)
        temb = get_timestep_embedding(time_cond, self.ngf)
        time_embed = self.time_embed(temb)
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model_res):
                ...
                if fourierfeat is not None: # then fuse 
                    ffout = self.ffmodel(fourierfeat)
                    cat_feat = torch.cat((feat, ffout), dim=1)
                    feat = self.fuse_model(cat_feat)
            ...
            # fuse fourier feat with the output of the resnet model (before upsampling)
            if fourierfeat.shape[0] < x.shape[0]:
                # Case when batch size of input is 2
                fourierfeat = fourierfeat.repeat(x.shape[0], 1, 1, 1)

            out = self.model(x)

            ffout = self.ffmodel(fourierfeat)
            cat_feat = torch.cat((out, ffout), dim=1)
            out = self.fuse_model(cat_feat)
...
class ResnetBlock_cond(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias,temb_dim,z_dim, fix_bug=True):
        """Initialize the Resnet block
