...
def get_new_visuals(opt, visuals):
    ...
    elif opt.dataset_mode=="aligned_npy":
...
if __name__ == '__main__':
    ...
    if opt.out_name is None:
        ...
        out_dir = opt.out_name
    web_dir = os.path.join(opt.results_dir, out_dir, '{}_{}'.format(opt.phase, opt.epoch))  # define the website directory
    print('creating web directory', web_dir)
    webpage = html.HTML(web_dir, 'Experiment = %s, Phase = %s, Epoch = %s' % (out_dir, opt.phase, opt.epoch))
    ...
    for i, (data,data2) in enumerate(zip(dataset,dataset2)):
        ...
        if (opt.dataset_mode == "unaligned_npy") or (opt.dataset_mode=="aligned_npy"):
            
            new_visuals = get_new_visuals(opt, visuals)
