...
from .ncsn_networks import NLayerDiscriminator_ncsn, ResnetGenerator_ncsn, ResnetGenerator_ncsn_FourierFeat, DPT_Generator
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[], debug=False, initialize_weights=True, netG_arch='resnet_9blocks_cond', freeze_dpt_backbone=False):
    ...
    if initialize_weights:
        if netG_arch == 'dpt':
            # Handle initialization using net.named_modules()
            # Avoid initializing the pretrained backbone model of DPT

            modules_to_init = ['model.scratch', 'model_res', 'model_upsample', 'z_transform', 'time_embed']

            for name, module in net.named_modules():

                if freeze_dpt_backbone:
                    if "pretrained" in name:
                        for param in module.parameters():
                            param.requires_grad = False

                if name in modules_to_init:
                    init_weights(module, init_type, init_gain=init_gain, debug=debug)
        else:
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'resnet_9blocks_cond_fourierfeat':
        net = ResnetGenerator_ncsn_FourierFeat(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    elif netG == 'dpt':
        net = DPT_Generator(input_nc, output_nc, ngf=128, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=opt.dpt_n_blocks, opt=opt)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netG)
    return init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netG), netG_arch=netG, freeze_dpt_backbone=opt.freeze_dpt_backbone)
